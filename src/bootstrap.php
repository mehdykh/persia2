...
function autoload($aClassName) {
  $classFile = "/src/{$aClassName}/{$aClassName}.php";
   $file1 = PERSIA_INSTALL_PATH . $classFile;
   $file2 = PERSIA_SITE_PATH . $classFile;
   if(is_file($file1)) {
      require_once($file1);
...
function exceptionHandler($e) {
  echo "Persia: Uncaught exception: <p>" . $e->getMessage() . "</p><pre>" . $e->getTraceAsString(), "</pre>";
}
...
function htmlent($str, $flags = ENT_COMPAT) {
  return htmlentities($str, $flags, CPersia::Instance()->config['character_encoding']);
}
