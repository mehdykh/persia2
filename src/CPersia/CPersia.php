...
/**
 * Main class for Persia, holds everything.
 *
 * @package PersiaCore
 */
class CPersia implements ISingleton {
  ...
  protected function __construct() {
    ...
    // include the site specific config.php and create a ref to $pe to be used by config.php
    $pe = &$this;
    require(PERSIA_SITE_PATH.'/config.php');
...
/**
        ...
        * @return CPersia The instance of this class.
        */
        public static function Instance() {
                if(self::$instance == null) {
                        self::$instance = new CPersia();
                }
  ...
  public function ThemeEngineRender() {
    ...
    $themePath         = PERSIA_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = PERSIA_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl        = $this->request->base_url . $this->config['theme']['parent'];
    }

    // Add stylesheet path to the $pe->data array
   $this->data['stylesheet'] = $this->config['theme']['stylesheet'];

    // Make the theme urls available as part of $pe
    ...
    $pe = &$this;
    // First the default Persia themes/functions.php
    include(PERSIA_INSTALL_PATH . '/themes/functions.php');
    ...
    // Extract $pe->data to own variables and handover to the template file
...
*/
  ...
  /**
* Draw HTML for a menu defined in $pe->config['menus'].
*
